...
contains 
  ...
  subroutine get_cost_function(fvec, ETBA_FIT, EDFT, neig, iband, nband, PINPT, nkpoint, PWGHT, ldjac, imode, flag_order, fvec_plain)
    ...
    real*8                     cost_func_plain(nband*PINPT%nspin)
    real*8                     enorm
    external                   enorm
...
!!!! TO BE CHECK !!!!
!   Wavefunction information ETBA_FIT%V is only properly stored in the master node #0
!   Hence, before evaluating cost function that uses "V", the information should be 
!   redistributed to the slave nodes. Aug. 05, 2020 KHJ
          ...
          do is = 1, PINPT%nspin
            do ie = 1, nband
              ...
              if(flag_order) then
                ...
                dE        = (ETBA_FIT%E(ie+(is-1)*nband,ik) - EDFT%E(ie_,ik))    * PWGHT%WT(ie_,ik)
                dE_plain  = (ETBA_FIT%E(ie+(is-1)*nband,ik) - EDFT%E(ie_,ik)) 
              endif
              ...
              cost_func_plain(ie+(is-1)*nband) = abs(dE_plain) 
            enddo
          enddo
          fvec(ik) = enorm ( nband*PINPT%nspin, cost_func )
          fvec_plain(ik) = enorm( nband*PINPT%nspin, cost_func_plain )
        enddo

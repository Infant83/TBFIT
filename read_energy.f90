...
subroutine read_energy_tbfit_V(E, V, ne_found, ispin, nspin, norb, nband, nk, kmode, flag_vector, flag_wf)
   use parameters, only : pid_energy
   implicit none
   integer*4              ispin, nspin, norb, nband, nk
   integer*4              ispinor
   ...
   complex*16             V(norb*ispin,nband*nspin,nk)
   real*8                 kpoint
   character*132          inputline
   ...
   do is = 1, nspin
     ...
     im = 1+norb*(is-1)  ; fm = norb*ispinor +norb*(is-1)
     call load_band_structure_V(E(ii:fi,:), V(im:fm,ii:fi,:), ne_found(is,:), ispinor, ispin, nspin, nband, norb, nk, kmode, pid, &
                              flag_vector)
   ...
   return
endsubroutine
subroutine read_energy_tbfit_V2(E, V2, ne_found, ispin, nspin, norb, nband, nk, kmode, flag_vector, flag_wf)
   use parameters, only : pid_energy
   implicit none
   integer*4              ispin, nspin, norb, nband, nk
   integer*4              ispinor
   ...
   do is = 1, nspin
     ...
     ii = 1+nband*(is-1) ; fi = nband+nband*(is-1)
     im = 1              ; fm = norb
     call load_band_structure_V2(E(ii:fi,:), V2(im:fm,ii:fi,:), ne_found(is,:), ispin, nspin, nband, norb, nk, kmode, pid, &
                              flag_vector)
...
subroutine load_band_structure_V(E,V,ne_found, ispinor, ispin, nspin, nband, norb, nk, kmode, pid, flag_vector)
   implicit none
   integer*4              pid
   integer*4              ie, ik, im
   integer*4              ispin,nband, norb, nk, ispinor, nspin
   integer*4              nskip
   integer*4              idummy
   integer*4              nitems
   ...
   character*132          inputline
   real*8                 E(nband, nk)
   complex*16             V(norb*ispinor, nband , nk)
   real*8                 V_(norb*ispinor*2)
   logical                flag_go, flag_vector
   character*4            kmode
       ...
       if(idummy .gt. 1) then
         ...
         do im = 1, norb
           if(ispinor .eq. 2) then
             ...
             V(im+norb, ie, ik) = cmplx( V_(im*4-1), V_(im*4) )
...
subroutine load_band_structure_V2(E, V2, ne_found, ispin, nspin, nband, norb, nk, kmode, pid, flag_vector)
   implicit none
   integer*4              pid
   integer*4              ie, ik
   integer*4              ispin,nband, norb, nk, ispinor, nspin
   ...
   do ie = 1, nband
     ...
     do ik = 1, nk
       ...
       if(idummy .gt. 1) then
         ...
         backspace(pid)
         if(flag_vector) then
           read(pid,*)kpoint(:), E(ie,ik),V2(1:norb,ie,ik)
         elseif(.not. flag_vector) then
           read(pid,*)kpoint(:), E(ie,ik)
         endif
       elseif(idummy .eq. 0) then
         ...
         read(pid,*)kpoint(:)
         E(ie,ik) = -999d0
         if(flag_vector) V2(1:norb,ie,ik) = 0d0
       endif
     enddo
   enddo

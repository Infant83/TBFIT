...
subroutine read_kpoint(PKPTS, PINPT, PGEOM)
  ...
  character*8,external :: str2upcase

  ! read info: kpoint 
  inquire(file=PKPTS%kfilenm,exist=flag_kfile_exist)
  if(flag_kfile_exist) then
    if( trim(PKPTS%kline_type) .eq. 'FLEURMAX') then
      call read_kpoint_file_fleur_MaX(PKPTS,PGEOM,PINPT%flag_ndiv_line_parse,PINPT%flag_ndiv_grid_parse)
    else
  ...
  return
endsubroutine
subroutine read_kpoint_file_fleur_MaX(PKPTS, PGEOM, flag_ndiv_line_parse, flag_ndiv_grid_parse)
    ...
    integer(kind=sp)                 i_continue, nitems,ndiv_temp
    ...
    character(len=132)               inputline,fname
    character(len=40 )               desc_str,dummy, k_name_dummy(max_kline)
    character(len=20 )               int2str
    character(len=*  ), parameter :: func = 'read_kpoint_fleur_MaX'
    logical       flag_skip
             ...
             elseif(linecount .eq. 3) then
               ...
               if(desc_str(1:1) .eq. 'L' .or. desc_str(1:1) .eq. 'l') then
                 ...
                 if(.not. flag_ndiv_line_parse) then
                   write(message,'(A,*(I8))')'   #N_DIV: (total number of ndiv)',PKPTS%n_ndiv  ; write_msgi
        ...
        ! dividing factor
         elseif(linecount .eq. 2) then
           ...
           if(ndiv_temp .ne. 0) then
             if(.not. allocated(PKPTS%ndiv)) allocate( PKPTS%ndiv(PKPTS%n_ndiv) )
             read(inputline,*,iostat=i_continue) PKPTS%ndiv(1:PKPTS%n_ndiv)
             if(PKPTS%n_ndiv .gt. 1) then 
               PKPTS%kline_type = 'FHI-AIMS' ! enforce 
             endif
           endif
           cycle

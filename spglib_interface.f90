! This module is copied from SPGLIB-1.10.4 (https://atztogo.github.io/spglib/index.html) 
! and slightly modified for the purpose. 
! by H.-J. Kim (KIAS, Infant@kias.re.kr)
#include "alias.inc"
module spglib_interface
   ...
   type :: SpglibDataset
      ...
      integer :: std_mapping_to_primitive
...
!   Beware the map refers to positions starting at 0
   ...
   function spg_get_dataset(lattice, position, types, num_atom, symprec) result(dset)
      ...
      type, bind(c) :: SpglibDataset_c
         ...
         integer(c_int) :: std_mapping_to_primitive
      ...
      if( c_associated(dataset_ptr_c)) then
         ...
         dset % std_mapping_to_primitive = dset_c % std_mapping_to_primitive
